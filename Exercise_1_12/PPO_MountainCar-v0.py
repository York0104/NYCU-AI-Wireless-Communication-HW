...
LOG_DIR = os.path.join(BASE_DIR, 'exp')
...
class PPO():
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f'Failed to initialize SummaryWriter: {exc}')
            self.writer = None
    ...
    def save_param(self):
        ...
        actor_path = os.path.join(NET_PARAM_DIR, f'actor_net_{timestamp}.pkl')
        critic_path = os.path.join(NET_PARAM_DIR, f'critic_net_{timestamp}.pkl')
        torch.save(self.actor_net.state_dict(), actor_path)
        torch.save(self.critic_net.state_dict(), critic_path)
        print(f'Saved model parameters to {NET_PARAM_DIR}')
...
def main():
        ...
        for t in count():
            ...
            if done:
                ...
                if agent.writer is not None:
                    ...
                    agent.writer.add_scalar('Reward/episode_reward', episode_reward, global_step=i_epoch)
                episode_rewards.append(episode_reward)
                moving_average = float(np.mean(episode_rewards[-reward_ma_window:]))
                moving_avg_rewards.append(moving_average)
                if agent.writer is not None:
                    agent.writer.add_scalar('Reward/moving_average', moving_average, global_step=i_epoch)
                if (i_epoch + 1) % args.save_interval == 0:
                    agent.save_param()
                print(
                    f'Episode {i_epoch} finished after {t + 1} steps | '
                    f'reward: {episode_reward:.1f} | '
                    f'moving_avg({reward_ma_window}): {moving_average:.2f}'
                )
                break
    ...
    reward_plot_path = os.path.join(IMG_DIR, 'episode_rewards.png')
    plt.tight_layout()
    ...
    plt.title(f'PPO on MountainCar-v0 ({reward_ma_window}-Episode Moving Average)')
    plt.grid(True, alpha=0.3)
    moving_avg_plot_path = os.path.join(IMG_DIR, 'moving_average_rewards.png')
    plt.tight_layout()
    plt.savefig(moving_avg_plot_path)
    plt.close()
    print(f'Saved moving average reward plot to {moving_avg_plot_path}')
